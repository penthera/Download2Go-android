...
class VideoPlayerActivity : Activity(),  PlaybackPreparer {
    ...
    // Required for advert playback - Exoplayer extension
    ...
    private fun initializePlayer() {
        ...
        if (player == null) {
            ...
            asset?.let{
                if(it is ISegmentedAsset && it.adSupport() == Common.AdSupportType.CLIENT_ADS){
                    var adsResponse: String? = null

                    val adsPackage: IVideoAdPackage? = virtuoso.assetManager.adManager.fetchAdsForAsset(it)
                    adsPackage?.let{ ad_package ->
                        adsResponse = ad_package.inlineAdResponse
                    }

                    adsResponse?.let{response->
                        if(response.isNotEmpty()){
                            ...
                            adsLoader = builder.buildForAdsResponse(response)
                            adsLoader!!.setPlayer(player)
                            val mediaSourceWithAds = AdsMediaSource(mediaSource, mediaDataSourceFactory, adsLoader, playerView)
    ...
    private fun buildDataSourceFactory(): DataSource.Factory =
        DefaultDataSourceFactory(applicationContext, DefaultHttpDataSourceFactory("download2go6"))
    ...
    companion object {
        ...
        private const val ACTION_VIEW = "com.penthera.download2go6.exoplayer.action.VIEW"
        ...
        fun playVideoDownload(asset: IAsset , context: Context){
            ...
            val path: Uri = Uri.parse(asset.playlist.toString())  //.playbackURL.toString())
