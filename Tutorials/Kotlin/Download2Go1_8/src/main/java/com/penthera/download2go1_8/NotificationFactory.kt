package com.penthera.download2go1_8
...
import com.penthera.common.Common.Notifications.EXTRA_NOTIFICATION_EVENT
...
class NotificationFactory(private val applicationName: String) {
    ...
    companion object {
        fun channelId() = "DOWNLOAD2GO_1.1_ID"
        fun channelName() = "Download2Go 1.1 Background Activity"
    ...
    fun getNotification(context: Context, intent: Intent?): Notification? {
        ...
        if (action.contains(NOTIFICATION_EVENT_TAG)) {

            val event: IEvent? = notificationIntent.getParcelableExtra(EXTRA_NOTIFICATION_EVENT)
