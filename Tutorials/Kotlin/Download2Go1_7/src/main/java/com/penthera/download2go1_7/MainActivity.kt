...
class MainActivity : AppCompatActivity() {
        ...
        if (virtuoso.backplane.authenticationStatus != AuthenticationStatus.AUTHENTICATED) { // If not authenticated then execute sdk startup
            ...
            if (name.isNotEmpty()) {// Here we use the simplest login with hard coded values
                ...
                virtuoso.startup(
                    ...
                    object : IPushRegistrationObserver { // Push registration observer (IPushRegistrationObserver)
                        override fun onServiceAvailabilityResponse(
                            pushService: Int,
                            connectionResponse: Int
                        ) {
    ...
    private fun unregister(){
        ...
        if (virtuoso.backplane.authenticationStatus == AuthenticationStatus.AUTHENTICATED) {
            try {
                virtuoso.backplane
                    .getDevices { iBackplaneDevices: Array<IDownloadDevice> ->
